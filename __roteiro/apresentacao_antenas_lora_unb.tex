...
\author{Projeto de antenas para botões de segurança}
...
\begin{itemize}
  ...
  \item Saída: $G_{\mathrm{parabólica}}(\alpha)$, não ganho máximo constante.
\end{itemize}
\end{columns}
\end{frame}
...
A escolha de antenas para botões de segurança LoRa/LoRaWAN no Campus Darcy Ribeiro deve ser baseada no papel da antena, na geometria real dos enlaces, no ganho efetivo por direção e na viabilidade de instalação.
\end{block}
\begin{itemize}
  \item A proposta wireless reduz dependência de cabeamento e conexões físicas frágeis.
  \item A posição do gateway é condicionada pela infraestrutura de segurança existente, não só pela centralidade geométrica.
  \item Antenas omnidirecionais são mais coerentes para gateway central único.
  \item Diretivas, inclusive parabólicas pequenas, podem ajudar em enlaces fixos apontados, mas trazem custo mecânico e menor cobertura angular.
  \item A solução mais robusta combina simulação, teste de campo e escolhas práticas de engenharia.
\end{itemize}
\end{frame}
